...
{\color{red} Author list not finalised. Should invite NXG
  and RFWS and others.}
...
using a single file format for all its data reduction software. The
...
model caused chaos and it took some time until order could be restored
with the definition of the \emph{N}-Dimensional Data Format (NDF).
...
Additionally, it is important to be able to copy individual
...
extension `\texttt{.sdf}'.  HDS files contain \emph{data objects} which will
...
types are `\texttt{IMAGE}', `\texttt{SPECTRUM}', `\texttt{INSTR\_RESP}'.
These do not begin with an underscore, so they are easily distinguished from the
primitive types, which do.

Special rules apply to the primitive types, which all begin with an
underscore and are ``pre-defined'' by HDS, as shown in
Table~\ref{tab:hdstypes}.
...
i.e.\ its number of dimensions is 0. A \emph{vector} has a
...
defined. It is represented as a logical value where \texttt{.TRUE.} means
defined and \texttt{.FALSE.} means undefined.
...
hierarchical structures. The design was flat and heavily
...
propagation model for NDF \citep[for example \textsc{specdre};][]{SUN140}.
...
share the data type of \texttt{NDF}. For example, the \textsc{gaia}
...
applications such as \textsc{smurf} \citep{2013MNRAS.430.2545C},
\textsc{ccdpack}, \textsc{gaia} and \textsc{kappa}.
