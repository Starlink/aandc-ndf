...
``PROVENANCE'' within the main NDF, and is encoded into an array of
...
library that are called when-ever an NDF is opened. The only changes that
then need be made to an application to enable basic provenance is for the
application to add two call to mark the start and end of a ``provenance
...
creaton of an output NDF. For instance, if NDF ``A'' is added to NDF
``B'' to create NDF ''C'', and ``A'' is then added to ``C'' to create
``D'', then ``A'' (and all its ancestors) would appear twice in the
provenance tree of ``D''. To avoid this, wheneve a new parent is added to
