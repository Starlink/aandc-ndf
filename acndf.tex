...
\address[noao]{LSST Project Office, 933 N.\ Cherry Ave, Tucson, AZ 85721, USA}
...
general purpose image formats such as JPEG2000 \citep{2014Kitaeff}. These
...
\citep{2000ASSL..250...93W} in the late 1980s
...
by discussing the areas where NDF could be improved.
Following this, in \secref{sec:social} we address the social,
political and economic considerations of data formats and models. In
\ref{app:chaos} we provide examples of data models devised in the
mid-1980s, the development of which motivated the creation of NDF
and also directly influenced the NDF design. A timeline describing the
key developments relating to NDF is shown in \tabref{tab:timeline}.

In this paper the term ``data model'' refers to the organization,
naming and semantics of components in a hierarchy. The term
...
\item Automatic type conversion allows a programmer to request that,
  say, a data array of 32-bit integers is accessed as 64-bit floating
  point numbers.
\end{itemize}

\begin{table}
\caption{Timeline for developments relating to NDF. Other
...
offsets so that modern large datasets can be processed\footnote{Support for
...
\caption{HDS basic data types. The unsigned types did not correspond
  ...
  used for the \textsc{generic} templating system \citep{SUN7}.}
\label{tab:hdstypes}
\begin{center}
\begin{tabular}{lll}
\hline
Name of type & API Code & Data type \\ \hline
...
\caption{Schematic of the NDF hierarchy. All components are optional
  except for \texttt{DATA\_ARRAY}. For an alternative visualization
...
structures or the semantics of the content. This resulted in
...
to the written specification \citep{SGP38} developed by the working group
...
each other is shown in \figref{fig:ndf-structure}. For more details
...
\citep[][\ascl{1409.010}]{2004SPIE.5498....1F}.
...
Comparing the history structure of \figref{fig:ndf-structure} with
that shown in \figref{fig:asterix}
...
The only other change involved the relatively recent addition to increase in resolution the
timestamp to support milliseconds. This was required as computers have
...
provenance tree is shown in \figref{fig:prov}.
...
the full AST-based WCS scheme, as described in \secref{sec:wcs}.
...
pixels 55 to 63 inclusive, and \verb|75~11| means 11 pixels centered
on Pixel 75.
...
information \citep[see for example,][for a discussion of this problem with FITS and HDF files]{1995ASPC...77..229J}.
The NDF model on HDS format files has always been one format amongst many
...
\emph{vice versa} \citep{SUN55,1997STARB..19...14C}.  In particular,
...
\emph{multispec} data model in IRAF \citep{1993ASPC...52..467V} to ensure
that wavelength scales were not lost. A description of how NDF maps to
FITS is given in \ref{app:ndf2fits}.
...
value, the comment and the unit. Alternatively, one might simply drop the unit
and comment and just use keyword/value pairs. These approaches turned
out to be extremely slow and space inefficient so the project took the  pragmatic approach
of standardizing on an array of characters formatted
identically to a FITS header.
This allowed the FITS to NDF conversion
software to ignore new FITS header conventions as they were developed,
...
example \figref{fig:duck}). The core NDF library thereby took an
...
that it did not understand. This ``duck typing''\footnote{See for
...
when rolling out updates: software linked with old library versions
...
ORAC-DR \citep[][\ascl{1310.001}]{2008AN....329..295C,2015oracdr}.  There can be many thousands of
...
not a funded goal and, indeed, the Project took few steps even to
monitor it.

A key consequence of this ``funding boundary'' was that Starlink staff
were not involved in discussions about data handling for new
...
most seasoned of standards organizations will struggle to reconcile
...
under an open-source license) followed what would now be recognized as
...
fundamental choices such as implementation language and software
dependencies can be critically important.
...
one and we will assume that such a layer will always be put in place,
...
requirements, so that they minimize their work. But this only applies
...
astronomical data formats could not develop in a similar, evolutionary
...
\citep[][\ascl{1402.007}]{2005ASPC..347...22D,2014Skoda}); however it
lost out to the more mainstream approach adopted by others
...
Centre, Hawaii. We thank Jim Peden and Trevor Ponman for providing
comments on the manuscript regarding the early days of HDS and the
development of \asterix. We also thank to the two anonymous referees
...
stored in \texttt{DATA\_QUALITY}. An example layout is given in \figref{fig:image}.
...
keywords were held in a similar structure labeled
...
labeled -- awkwardly -- from \texttt{T} through to \texttt{W}. An
example layout can be found in \figref{fig:dst}.
...
\figref{fig:asterix}. The \asterix\ data models were not competing directly with \emph{IMAGE} or
...
\figref{fig:duck} for an example).
...
An example of part of such a structure is shown in \figref{fig:hdstable}.
...
extensions. This example specifies that the table has four rows and three
...
keywords: \texttt{EXTNAME}, \texttt{EXTVER}, and \texttt{EXTLEVEL}.
Indeed \texttt{EXTLEVEL} was added by
...
writers have in effect used it as the type (cf.
\secref{sec:name_v_type}).  The second missing
...
\CFITSIO\ \citep[][\ascl{1010.001}]{1999ASPC..172..487P}
...
storage would be to create a single table for the array structure,
...
The following header items are set to enable recovery of the hierarchical
structure.
...
For the reverse operation, the FITS reader decides whether or not it
knows the semantics of the FITS file to map FITS extensions to NDF
components. Various products from known sources are recognized from
keyword values. In the case of a former NDF, the reader examines
the \texttt{HDUCLAS1} keyword.  If its value is \texttt{NDF}, the
reader endeavors to recreate the NDF components and extensions from
the keywords listed above, working through the FITS extensions in
order.

For an arbitrary FITS file there is no reason to expect that its data
model maps well to the NDF model.  The default behavior is for the
primary HDU to map to the NDF \texttt{DATA\_ARRAY}, in which blank and
\texttt{NaN} values become the NDF bad value; the WCS headers are used to form
an NDF \texttt{WCS} or \texttt{AXIS} component.  For many cases having
the primary array and metadata is adequate for the conversion.  It is
analogous to the early \emph{IMAGE}-model files operating as NDFs.
However, additional FITS extensions are preserved by default too,
being converted to a series of NDF extensions.  The HDS type of
each NDF extension depends on the \texttt{XTENSION} keyword.  It is
\texttt{NDF} for an IMAGE, and \texttt{TABLE} (\ref{app:hdstable}) for
BINTABLE and TABLE.  The latter representation is often not optimal,
as such extensions can be used to represent many different data
models.  An example is when a binary table holds a series of data
arrays observed at different locations such as from multi-object
spectroscopy.  It is possible with generic HDS tools to manipulate
such data into NDF form.  Where the user knows the mappings between
multi-extension FITS and NDF array components, the FITS reader has a
mechanism for specifying these mappings.

Full details of the conversions between NDF and FITS are available in
\citet{SUN55}.
