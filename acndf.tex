...
keywords: EXTNAME, EXTVER, and EXTLEVEL.  Indeed EXTLEVEL was added by
...
additional keyword EXTTYPE to preserve this information.  Note that
the definition of EXTNAME was somewhat terse and vague in
\citet{1988A&AS...73..359G} being just the 'name', and some FITS
...
serialization array components such as VARIANCE and QUALITY are
written to IMAGE extensions, whose headers retain information
stored in other top-level components such as WCS and LABEL.  Specially
formatted headers may be written to record the HISTORY, which if not
edited, can be read back into NDF HISTORY records.  This is not as
...
information, if present, is stored via five keywords, PRV[CDIMP]$n$
...
(Section~\ref{sec:FITSheaders}) are merged, but with NDF information
...
\item [EXTNAME]
Within an IMAGE extension this is the name of an array component, since
these are in well-defined locations within the NDF. Within a binary
table, which could record arbitrary NDF structures, EXTNAME stores the
...
too long for the 68 characters in a FITS header EXTNAME is set to a
...
using the HEASARC long-string CONTINUE
convention.\footnote{\texttt{http://fits.gsfc.nasa.gov/registry/continue\_keyword.html}}
